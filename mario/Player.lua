    ...
    self.behaviors = {
        ...
        ['winner'] = function(dt)
            ...
            if self.map:collides(self.map:tileAt(self.x, self.y + self.height)) or
                ...
                self.flag.state = 'flag_fly'
                    
                if self.flag.y > map.tileHeight * ((map.mapHeight - 2) / 2) - self.height then
                    self.flag.y = map.tileHeight * ((map.mapHeight - 2) / 2) - self.height
                    self.flag.state = 'flag_down'
                    self.dy = 0
                end
...
function Player:calculateJumps()
    ...
    if self.dy < 0 then
        if self.map:tileAt(self.x, self.y).id == JUMP_BLOCK or
...
-- checks player has fallen off/went above the map
function Player:checkFall()
    if self.y > VIRTUAL_HEIGHT or self.y < (0 - self.height) then
        love.event.quit("restart")
    end
end

-- checks player has gone out of the map
function Player:checkBoundary()
    -- Check right side of the map
    if self.x > map.mapWidthPixels - map.tileWidth * 2 then
        self.x = (self.map:tileAt(self.x + self.width, self.y).x - 1) * self.map.tileWidth - self.width
    -- Check left side of the map
...
function Player:checkWin()
    if self.dx > 0 then
        if self.map:wins(self.map:tileAt(self.x + self.width, self.y)) or
            self.map:wins(self.map:tileAt(self.x + self.width, self.y + self.height - 1)) or
            self.map:wins(self.map:tileAt(self.x - 1, self.y)) or
            self.map:wins(self.map:tileAt(self.x - 1, self.y + self.height - 1))  then
            
            -- set state to winner to prevent movement
            self.state = 'winner'
        end
    end
end
